...
struct LoginViewModel {
  static func authInitiate(username: String, callback: @escaping (_ json: [String: AnyObject]?, _ error: String?) -> () ) {
    let contentType = APIHTTPHeader.contentType
    let authInitiateEndPoint = AuthEndpoint.initiate
    let authRequest = NetworkAPIRequestFor(endpoint: authInitiateEndPoint, headers: [contentType], parameter: ["username" : username])
