...
struct SignUpViewModel {
  ...
  static func registerInitiate(username: String, callback: @escaping (_ json: [String : AnyObject]?, _ error: String?) -> ()) {
    let contentType = APIHTTPHeader.contentType
    let registerInitiateEndPoint = RegisterEndpoint.fidoRegisterInitiate
    let registerInitiateRequest = NetworkAPIRequestFor(endpoint: registerInitiateEndPoint, headers: [ contentType], parameter: ["username" : username])
